# SOURCE: https://github.com/BrianPulfer/PapersReimplementations/blob/main/ddpm/ddpm.py
...
def generate_new_images(ddpm, n_samples=16, device=None, frames_per_gif=100, gif_name="sampling.gif", c=1, h=28, w=28):
    ...
    with imageio.get_writer(gif_name, mode="I") as writer:
        for idx, frame in enumerate(frames):
            frame2 = np.squeeze(frame)  # Remove the extra dimensions if present
            frame2 = np.repeat(frame2[:, :, np.newaxis], 3, axis=2)
            writer.append_data(frame2)
            if idx == len(frame2) - 1:
                for _ in range(frames_per_gif // 3):
                    writer.append_data(frame2[-1])
    return x
